...
- **Reviews are built for a hundred, not for three.** [ProductReviews](resources/js/storefront/components/ProductReviews.vue) is a full-width section below the product (the buy column keeps only [ProductRatingSummary](resources/js/storefront/components/ProductRatingSummary.vue) — one line and a heart, so the Add to Bag button never gets pushed off-screen). It follows Baymard's five requirements for a ratings distribution: a bar chart, **the bars ARE the star filters**, open by default, radio-like rather than checkboxes, and hidden below `BREAKDOWN_FROM` ratings. Sorting (recent · highest · lowest), star filtering and paging are all the SERVER's (`ProductEngagement::comments()`), so paging can never disagree with the sort; it's **offset paging on purpose** — re-sorting invalidates a keyset cursor, and one product's reviews are a bounded set. A review is shown with its writer's own stars (a LEFT join — a review without a rating still shows), and long ones clamp with "Read more".
- The two components share one summary through the `blessluxe:product-engagement` window event, so a heart tapped beside the buy button moves the count in the reviews section without either re-fetching.
