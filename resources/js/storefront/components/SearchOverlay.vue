...
<template>
    <transition name="search-fade">
        <div v-if="open" class="fixed inset-0 z-50 bg-cream/95 backdrop-blur overflow-y-auto overscroll-contain pb-10" @click.self="close">
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
                <div class="flex items-center justify-between mb-6">
                    <p class="font-script text-2xl text-gold">Find</p>
                    <button @click="close" class="w-11 h-11 -mr-2 inline-flex items-center justify-center hover:text-gold transition-colors" aria-label="Close search">
                        <X class="w-5 h-5" />
                    </button>
                </div>
