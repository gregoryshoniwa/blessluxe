...
<template>
    <div class="fixed inset-0 z-[90] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true" aria-label="Schedule a live">
        ...
        <form @submit.prevent="save" class="relative bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92dvh] flex flex-col shadow-2xl">
            ...
            <div class="flex-1 min-h-0 overflow-y-auto overscroll-contain px-5 py-4 space-y-4">
                <p class="text-xs text-black/55 bg-cream rounded-xl px-3.5 py-3 leading-relaxed">Go live on YouTube, TikTok, Facebook or Instagram as you normally do, and paste that link here. People watch it inside the Hive, get a reminder when you start, and can send you gifts.</p>
                <label class="block"><span class="block text-xs text-black/60 mb-1">What's it about?</span>
                    <input v-model.trim="form.title" required maxlength="100" placeholder="Styling five wedding-guest looks" class="w-full border border-black/12 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold" />
                ...
                <label class="block"><span class="block text-xs text-black/60 mb-1">Your live's link</span>
                    <input v-model.trim="form.url" required type="url" inputmode="url" autocapitalize="none" placeholder="https://…" class="w-full border border-black/12 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold" />
                ...
                <label class="block"><span class="block text-xs text-black/60 mb-1">When</span>
                    <input v-model="form.starts_at" required type="datetime-local" class="w-full border border-black/12 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold" />
                ...
                <div><span class="block text-xs text-black/60 mb-1.5">How will you film it?</span>
                    <div class="grid grid-cols-3 gap-2">
                        <button v-for="sh in shapes" :key="sh.key" type="button" @click="form.shape = sh.key" :aria-pressed="form.shape === sh.key" :class="['rounded-xl border py-2.5 flex flex-col items-center gap-1 text-xs transition-colors', form.shape === sh.key ? 'border-gold bg-cream' : 'border-black/10 text-black/55']">
                ...
                <label class="block"><span class="block text-xs text-black/60 mb-1">Anything else? <span class="text-black/35">(optional)</span></span>
                    <textarea v-model="form.description" rows="2" maxlength="400" class="w-full border border-black/12 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold resize-none"></textarea></label>
            ...
            <footer class="px-5 py-3.5 border-t border-black/8 flex-shrink-0 pb-[max(0.875rem,env(safe-area-inset-bottom))]">
                <button type="submit" :disabled="saving" class="w-full bg-gold text-white py-3.5 rounded-full text-xs font-semibold tracking-[0.3em] uppercase hover:bg-gold-dark disabled:opacity-40 inline-flex items-center justify-center gap-2">
