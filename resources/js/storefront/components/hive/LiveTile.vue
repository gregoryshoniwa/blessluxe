...
<template>
    <router-link :to="`/hive/live/${live.id}`" class="block bg-white border border-black/8 rounded-2xl overflow-hidden hover:border-gold transition-colors">
        <span class="relative block aspect-video bg-gradient-to-br from-zinc-800 to-black">
            ...
            <span v-if="live.state === 'live'" class="absolute top-2 left-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-600 text-white text-[10px] font-semibold tracking-widest uppercase"><span class="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span> Live</span>
            <span v-else-if="live.state === 'upcoming'" class="absolute top-2 left-2 px-2.5 py-1 rounded-full bg-black/70 text-white text-[10px] tracking-wide">{{ when }}</span>
            <span v-else class="absolute top-2 left-2 px-2.5 py-1 rounded-full bg-black/70 text-white text-[10px] tracking-widest uppercase">Watch again</span>
            <span class="absolute bottom-2 right-2 px-2 py-0.5 rounded-full bg-black/60 text-white text-[10px]">{{ live.embed?.label }}</span>
        </span>
        <span class="flex items-center gap-2.5 p-3">
            <span class="w-8 h-8 rounded-full overflow-hidden bg-cream-dark flex items-center justify-center flex-shrink-0">
