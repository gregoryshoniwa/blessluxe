...
<template>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-4 lg:py-8">
        <div class="flex items-center justify-between gap-3 mb-5">
            ...
            <button @click="schedule" class="h-10 px-5 bg-gold text-white text-xs font-semibold tracking-[0.15em] uppercase hover:bg-gold-dark inline-flex items-center gap-1.5"><Plus class="w-4 h-4" /> Schedule</button>
        </div>
        ...
        <div v-else-if="empty" class="text-center py-16">
            ...
            <button @click="schedule" class="bg-gold text-white px-8 py-3.5 text-xs font-semibold tracking-[0.25em] uppercase hover:bg-gold-dark">Schedule a live</button>
        </div>
